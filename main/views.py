...
class MenuListView(ListView):
    ...
    queryset = Item.objects.filter(label_colour='success')
    context_object_name = 'menu_items'

def side(request):
     item = Item.objects.filter(label_colour='info')
     context = {
        'item' : item
     }
     return render(request, 'main/side.html', context)

def menuDetail(request, slug): 
    item = Item.objects.filter(slug=slug).first()
    reviews = Item.objects.filter(label_colour='primary')
    context = {
        'item' : item,
        'sides' : reviews,
    }
...
def add_to_cart(request, slug):
    ...
    return redirect("main:side")

@login_required
def add_to_carta(request, slug):
    item = get_object_or_404(Item, slug=slug)
    cart_item = CartItems.objects.create(
        item=item,
        user=request.user,
